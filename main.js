var simplexApp = new Vue({
    ...
    computed: {
        ...
        steps: function() {
            ...
            let selected_col = tmp[last_row].indexOf(min);
            /*
            ...
            for(let i = 0; i < tmp.length; i++)
                if(typeof tmp[i][B] == 'number')
                    tmp[i][B] = tmp[i][H]/tmp[i][selected_col];
            steps.push(tmp);
            //
            tmp = copyMatr(tmp);
            ...
            resolving_el.col = selected_col - 1; //-1 тк преебразуем jстолбец (X1)  в данные удобные для прользователя
            tmp[resolving_el.row][selected_col] = resolving_el.col;
            alert( resolving_el.row + ' ' + selected_col +' ' +  resolving_el.col);
            steps.push(tmp);
            //
            return steps;
        }
